...
Пример:
```
FRAME "Пользовательские роли" 100 100 800 400 blue
```
...
Пример:
```
SHAPE "Администратор" 200 150 150 80 red rectangle
SHAPE "HR-менеджер" 400 150 150 80 green
```
...
```
- `текст` - содержимое стикера
- `x, y` - координаты центра
- `цвет` - опционально (yellow, blue, green, pink, orange, purple, gray)

Пример:
```
STICKY "Важная заметка" 300 200 yellow
```

#### LINK - Создание связи
```
LINK <элемент1> <элемент2> [подпись] [тип]
```
...
FRAME "Архитектура системы" 50 50 1200 800 blue
...
SHAPE "Frontend" {current_x} {current_y} 200 100 green rectangle
SET current_x {current_x + spacing_x}
SHAPE "Backend" {current_x} {current_y} 200 100 red rectangle
SET current_x {current_x + spacing_x}
SHAPE "Database" {current_x} {current_y} 200 100 purple rectangle

# Создание связей
LINK "Frontend" "Backend" "API calls"
LINK "Backend" "Database" "SQL queries"

# Добавление заметок
STICKY "Версия 1.0" 100 300 yellow
